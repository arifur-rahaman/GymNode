...
- **Nav items not built yet** (ওয়েবসাইট, etc.) are shown with a "শীঘ্রই" (coming soon) tag *(see Q9)*.
...
### M0 — Setup
- [ ] pnpm + Turborepo monorepo (`apps/web`, `packages/core`, `packages/db`, `packages/config`)
- [ ] Next.js 16 app, TypeScript strict, ESLint + Prettier
- [ ] Supabase CLI local dev (`supabase start`), first empty migration, generated types
- [ ] Tailwind v4 + shadcn/ui themed with `tokens.css`; fonts via `next/font`
- [ ] next-intl with `bn` (default) + `en`, language switch
- [ ] Theme toggle (dark default) with no flash
- [ ] `packages/core`: money/lakh formatting + Dhaka date helpers **with unit tests**
- [ ] Vitest + Playwright (one smoke test)
- [ ] GitHub Actions CI: lint, type-check, unit tests, RLS tests
- [ ] `/dev/ui` gallery: Button, Input, Select, Badge, KpiCard, Card, Tabs/Segmented, Toggle, Table→Card list, Sheet, Toast, EmptyState
